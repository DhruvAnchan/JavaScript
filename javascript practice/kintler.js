...
const runMissionControl = async (minMissions = 2) => {
    ...
    // Here, we're creating an object that maps mission years to the number of missions in that year.
    const missionsByYear = missionData.reduce((accumulator, mission) => {
        // CONCEPT: Object Destructuring from a parameter
        const { year } = mission;
        // CONCEPT: Short-circuiting with ||
        // If accumulator[year] is falsy (undefined), it uses 0.
        accumulator[year] = (accumulator[year] || 0) + 1;
        return accumulator;
    }, {}); // {} is the initial value of the accumulator.

    console.log('\n--- Mission Analytics ---');
    console.log('Mission count by year:', missionsByYear);


    // CONCEPT: Spread Operator (...)
    // Expands an iterable (like an array or object) into individual elements.
    const firstAstronaut = astronauts[0];
    const firstAstronautCopy = { ...firstAstronaut, status: 'Retired' };
    console.log(`\nOriginal astronaut object:`, firstAstronaut);
    console.log('Copied and modified astronaut object:', firstAstronautCopy);
